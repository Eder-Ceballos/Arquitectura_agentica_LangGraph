...
def create_graph():
    ...
    # 3. La Bifurcación (Conditional Edge)
    # Aquí el grafo "pregunta" qué camino tomar basándose en el state["es_valido"]
    workflow.add_conditional_edges(
        "validator",
        should_continue,
        {
            "buscar_vacantes": "vacancy_scraper", # Camino A: Todo bien, busca empleo
            "detener": END                        # Camino B: Faltan datos, aborta misión
        }
    )

    # 4. Si llegó a buscar vacantes, ese es el final del proceso (por ahora)
